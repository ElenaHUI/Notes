...
> - **ITL**（Inter-Token Latency，出字间隔）：decode 阶段相邻两个 token 的间隔，决定出字快慢；$1000/ITL$ ≈ 每秒出字数。
> - **E2E**（端到端总时长）：一个请求从进入到说完的总耗时，近似满足 $E2E \approx TTFT + ITL \times OSL$。
...
| 参数名                                       | 单位                  | 计算公式                                                                                             | 详细解释                                                       |
| ----------------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- |
| Frontend Requests / Sec                   | 无（实为 req/s）         | $RPS=\dfrac{\Delta N_{req}}{\Delta t}\cdot\mathcal{R}$，$\Delta t=30\text{s}$                     | 前端请求速率，是负载的度量，其余曲线都要对照它判读。                                 |
| Frontend Avg Time to First Token          | 无（实为 ms，查询里已 ×1000） | $\overline{TTFT}=1000\times\dfrac{\Delta\sum_i TTFT_i}{\Delta N_{req}}\cdot\mathcal{R}$          | 平均首 token 延迟，含 prefill 排队 + GPU 计算 + NIXL 传输               |
| Frontend Avg Request Duration             | 无（实为 ms）            | $\overline{E2E}=1000\times\dfrac{\Delta\sum_i E2E_i}{\Delta N_{req}}\cdot\mathcal{R}$            | 平均端到端请求耗时。应满足 $E2E\approx TTFT+ITL\times OSL$              |
| Frontend Avg Inter-Token Latency          | 无（实为 ms）            | $\overline{ITL}=1000\times\dfrac{\Delta\sum ITL}{\Delta N_{gap}}\cdot\mathcal{R}$                | 平均 token 间延迟，即 decode 出字速度。                                |
| Frontend Avg Input/Output Sequence Length | 无（token 数）          | $\overline{ISL}=\dfrac{\Delta\sum ISL}{\Delta N_{req}}$，$\overline{OSL}$ 同理，均 $\cdot\mathcal{R}$ | 两条线：输入长度 ISL 与输出长度 OSL。故障。                                 |
| **Frontend Queued Requests**              | short（个，瞬时值）        | $N_{queued}(t)=\sum_{pod}\ \sum_{s\in\{preprocess,\ route,\ dispatch\}} N_s(t)\cdot\mathcal{R}$  | 排队等待的请求数（三阶段之和，按 pod 聚合）。面板说明称其为诊断 prefill worker 瓶颈的第一指标。 |
...
| 参数名                                       | 单位    | 计算公式                                                                                                                                | 详细解释                                                                                                                                                                               |
| ----------------------------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Prefill Worker Processing Time            | ms    | $\overline{D_p}=1000\times\dfrac{\Delta\sum D_p}{\Delta N_p}$；$P_{99}(D_p)$ 取自直方图桶，按 pod 分线，均 $\cdot\mathcal{R}$                    | prefill worker 处理耗时（Avg + P99），含 KV cache 经 NIXL 传输的时间。                                                                                                                            |
| Prefill Worker Throughput                 | reqps | $r_p=\dfrac{\Delta N_p}{\Delta t}\cdot\mathcal{R}$，按 pod 分线                                                                         | 各 prefill worker 的请求吞吐（仅统计 generate 端点）。各 pod 量级接近 = 负载均衡良好；某 pod 显著低 = 未被有效调度或已异常；总和应与前端 RPS 匹配。                                                                                  |
| **Component Latency - Prefill vs Decode** | s     | 同图两条：$\overline{D_p}=\dfrac{\Delta\sum D_p}{\Delta N_p}$ 与 $\overline{D_d}=\dfrac{\Delta\sum D_d}{\Delta N_d}$，均 $\cdot\mathcal{R}$ | 全盘最有价值的面板。健康表现：两侧耗时处于同一量级、比例稳定。prefill 远高于 decode → prefill 是瓶颈（扩 prefill / 减小 chunk）；decode 远高 → decode 拥挤（扩 decode / 查 KV cache 利用率）。⚠️ decode worker 的组件名是 `backend`，不叫 decode。 |
...
| 参数名                                  | 单位             | 计算公式                                                                | 详细解释                                                                                                |
| ------------------------------------ | -------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Decode Worker - Request Throughput   | reqps          | $r_d=\dfrac{\Delta N_d}{\Delta t}\cdot\mathcal{R}$                  | 各 decode worker 的请求吞吐。各 pod 均衡、随负载平滑变化为好。                                                           |
| Decode Worker - Avg Request Duration | s              | $\overline{D_d}=\dfrac{\Delta\sum D_d}{\Delta N_d}\cdot\mathcal{R}$ | 各 decode worker 平均处理耗时。各 pod 曲线重叠、离散度小为好；单个 pod 突出 = 坏 worker（GPU 降频 / KV 打满 / 邻居干扰），可直接拿 pod 名定位。  |
| **KV Cache Utilization**             | percentunit    | $U_{kv}(t)\cdot\mathcal{R}$（瞬时值，直接读取）                               | decode worker 的 KV cache GPU 显存占用率。理想稳定在 **0.6~0.8**：太低=显存浪费，>0.9 = decode 容量见底、batch 被压缩、ITL 随之恶化。 |
| KV Cache Blocks (Total)              | short（block 数） | $B_{total}(t)\cdot\mathcal{R}$（瞬时值，直接读取）                            | decode worker 上 KV cache 的总容量。是常量参照线，正常应为水平直线；突然变化 = worker 重启/重新分配显存。                              |
...
| 参数名                      | 单位                       | 计算公式                                                                                     | 详细解释                                                                                                                                                       |
| ------------------------ | ------------------------ | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GPU Compute Utilization  | percent（0~100）           | $U_{sm}(t)$（瞬时值）                                                                         | GPU SM 计算利用率。prefill GPU 应在 prefill 期间接近 100%；decode GPU 偏低是正常的。                                                                                           |
| GPU Memory Used          | gbytes（GB）               | $M_{used}=\dfrac{M_{MiB}}{1024}$                                                         | GPU 显存占用。应平稳且留有余量（离显存上限有 10% 以上空间）。持续贴顶 = OOM 风险；锯齿状剧烈波动 = KV block 反复分配释放。                                                                                |
| **GPU Memory Bandwidth** | percent（0~100）           | $U_{mem}(t)$（瞬时值）                                                                        | 显存拷贝带宽利用率，尖峰即 NIXL 的 KV cache 传输。健康表现：只在 KV 传输时出现尖峰、之后回落；持续 >80% = 传输已成瓶颈。                                                                                 |
| **NVLink Bandwidth**     | GBs（GB/s，Y 轴 0~50，2 位小数） | $BW_{nvl}=\dfrac{\Delta B_{tx}+\Delta B_{rx}}{\Delta t\times 10^9}$，$\Delta t=1\text{m}$ | 每 GPU 的 NVLink 双向总带宽。，但要两头看：**< 1 GB/s 反而是坏信号**——面板说明写明这代表跨 pod 的 NIXL KV 传输退化成了走主机内存拷贝，没吃到 NVLink/GPUDirect；> 35 GB/s 则接近 NVLink 饱和，健康区间大致 **1~20 GB/s**。 |
...
| 参数名                       | 单位                | 计算公式                                                                                            | 详细解释                                                                                                      |
| ------------------------- | ----------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| **Worker CPU Usage**      | short（CPU 核数）     | $C_{cpu}=\dfrac{\Delta t_{cpu}}{\Delta t}\cdot\mathcal{R}$                                      | worker pod 的 CPU 用量，单位就是核数。应远低于 pod 的 CPU limit；打到 limit 说明 CPU 成了瓶颈（tokenize / 采样 / 调度抢不到 CPU，会拖累 TTFT）。 |
| Node CPU Utilization      | percent（0~100，堆叠） | $U_{node}=100-100\times\overline{\left(\dfrac{\Delta t_{idle}}{\Delta t}\right)}$，按 instance 聚合 | 节点整体 CPU 利用率（用 idle 反推）。**< 70% 为好**，留出突发余量。                                                              |
| Worker Request Throughput | short（实为 req/s）   | $r_c=\dfrac{\Delta N_c}{\Delta t}\cdot\mathcal{R}$，按组件 × pod 分线                                 | 唯一同时显示 prefill 与 decode 吞吐的面板（仅统计 generate 端点）。健康表现：$r_p\approx r_d$                                      |
| Worker Data Transfer      | Bps（字节/秒）         | IN：$\dfrac{\Delta B_{req}}{\Delta t}$；OUT：$\dfrac{\Delta B_{resp}}{\Delta t}$                   | worker 的入/出字节速率（IN=请求、OUT=响应）。应与请求量、ISL/OSL 同步变化，用来交叉验证报文大小是否正常；IN 突增而 RPS 未涨 = 来了超大 prompt。              |

### 相关参数

| 关系式 | 健康表现 | 违背时的含义 |
|---|---|---|
| $E2E \approx TTFT + ITL \times OSL$ | 两边接近 | 左边明显偏大 → 存在额外排队/调度开销 |
| $\overline{D_p}$ vs $\overline{D_d}$（Component Latency） | 同一量级、比例稳定 | 哪侧高哪侧就是瓶颈：prefill 高 → 扩 prefill/减小 chunk；decode 高 → 扩 decode/查 KV 利用率 |
| $r_p \approx r_d$ | 两条速率守恒 | 不等 → P/D 流水线中间有堆积或丢弃 |
| $N_{queued}$ 与 $\overline{TTFT}$ | 同步且都低 | queued 持续 >0 并上升 → prefill worker 已饱和 |
| $U_{kv}$ 与 $\overline{ITL}$ | $U_{kv}\in[0.6,0.8]$ 且 ITL 平直 | $U_{kv}>0.9$ 伴随 ITL 抬升 → decode 容量见底，batch 被压缩 |
| $BW_{nvl}$ 与 $U_{mem}$ | NVLink 有量（1~20 GB/s）、显存带宽只见尖峰 | $BW_{nvl}<1$ 而 $U_{mem}$ 持续高 → NIXL 退化成主机内存拷贝 |
...
| 参数名                  | 单位                         | 计算公式                                       | 详细解释                                                                                                                 |
| -------------------- | -------------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| Prefill Workers      | none（个，stat 取 lastNotNull） | $N_p(t)$                                   | 当前 prefill worker 副本数                                                                                                |
| Decode Workers       | none（个，stat 取 lastNotNull） | $N_d(t)$                                   | 当前 decode worker 副本数                                                                                                 |
| Cumulative GPU Hours | h（小时，2 位小数）                | $H_{gpu}(t)=\displaystyle\int N_{gpu}\,dt$ | planner 启动以来累计消耗的 GPU 小时，单调递增即正常。用途是算成本：$\dfrac{H_{gpu}\times 单价}{T_{out}}$ = 每 token 成本，同等产出下越低越好；斜率突增而吞吐没涨 = 扩容浪费。 |
| Worker Count History | none（个，decimals=0）         | $N_p(t)$、$N_d(t)$ 同图两条时序                   | 副本数的变化过程。阶梯平缓、变化次数少为好；锯齿状频繁扩缩（flapping）说明 planner 阈值设得太敏感，会反复付冷启动开销。                                                 |

### Row 2 · Observed Metrics（planner 实际观测到的，3 项）

| 参数名 | 单位 | 计算公式 | 详细解释 |
| --- | --- | --- | --- |
| **Observed Latency (TTFT & ITL)** | ms | 同图 4 条：$TTFT_{obs}$、$ITL_{obs}$ 与目标线 $TTFT_{sla}$、$ITL_{sla}$ | ⭐ 全盘最重要：两条实测线应持续位于对应 SLA 线之下。越贴近 SLA 线说明资源利用越充分（不浪费）；越线 = 违约，planner 应立即扩容；长期远低于 SLA = 过度配置，可缩容省钱。 |
| Observed Request Rate & Duration | 无（两条线量纲不同） | 同图两条：$RPS_{obs}$ 与 $\overline{E2E}_{obs}$ | 实测请求速率与请求耗时。Rate 平稳或缓变为好，Duration 应与 $TTFT+ITL\times OSL$ 大致吻合。⚠️ 面板未设 unit，且把 req/s 与秒放在同一 Y 轴，量级差会压平其中一条 —— 读数请看 legend 的 lastNotNull/mean/max，别看曲线形状。 |
| Observed Sequence Lengths (ISL & OSL) | none（token 数） | 同图两条：$\overline{ISL}_{obs}$ 与 $\overline{OSL}_{obs}$ | 实测输入/输出长度，是 planner 做预测的输入特征。稳定为好，剧烈漂移会直接导致 Row 3 的预测失准。 |

### Row 3 · Predicted Metrics（planner 的预测输出，3 项）

| 参数名                                    | 单位                 | 计算公式                                                   | 详细解释                                                                               |
| -------------------------------------- | ------------------ | ------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| Predicted Request Rate                 | reqps              | $RPS_{pred}(t)$，对照 $RPS_{obs}$ 看                       | 预测的请求速率。判据是与 Row 2 的实测速率对比：预测曲线应略微领先实测且形状吻合。滞后 = 预测无效（扩容总慢一步）；偏差大 = 预测模型不适配当前流量模式。 |
| Predicted Sequence Lengths (ISL & OSL) | token 数            | 同图两条：$\overline{ISL}_{pred}$ 与 $\overline{OSL}_{pred}$ | 预测的输入/输出长度。与 Row 2 的实测值贴合为好；系统性偏高 → 会过度扩容，偏低 → 容量不足、SLA 有风险。                       |
| Predicted Replica Counts               | none（个，decimals=0） | 同图两条：$N_p^{pred}$ 与 $N_d^{pred}$                       | 预测所需的副本数。与 Row 1 的实际副本数贴合 = planner 的决策被正常执行                                       |
...
| 参数名                           | 单位         | 计算公式                                                                                             | 详细解释                                                       |
| ----------------------------- | ---------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- |
| Reconciliation Rate           | reqps（次/秒） | $r_{rec}=\dfrac{\Delta N_{rec}}{\Delta t}$，按 $(type,\ result)$ 分线，$result\in\{success,\ error\}$ | 控制器主循环的执行速率。平稳的低速率且 success 占绝对主导为好，空闲期应接近 0（只有周期性 resync） |
| Reconciliation Duration (P95) | s          | $P_{95}(D_{rec})$，按 $type$ 分线                                                                    | 单次 reconcile 的耗时分布上沿。亚秒级（< 1s）且平稳为好                        |
| **Reconciliation Errors**     | reqps      | $r_{err}=\dfrac{\Delta N_{err}}{\Delta t}$，按 $(type,\ err)$ 分线                                   | reconcile 的报错速率，理想恒为 0                                     |
...
| 参数名 | 单位 | 计算公式 | 详细解释 |
| --- | --- | --- | --- |
| Webhook Request Rate | reqps | $r_{wh}=\dfrac{\Delta N_{wh}}{\Delta t}$，按 $(type,\ op,\ result)$ 分线，$op\in\{CREATE,\ UPDATE,\ DELETE\}$ | 准入请求速率。与你的部署操作频率相符即正常，$result=allowed$ 应占绝对多数、空闲期接近 0。突然大量 UPDATE = 有控制器在反复改同一对象。 |
| Webhook Duration (P95) | s | $P_{95}(D_{wh})$，按 $type$ 分线 | 准入校验耗时，必须是毫秒级（远小于 1s）。它同步阻塞在 API Server 的调用路径上，变慢会直接拖慢所有 `kubectl apply`；接近 webhook timeout（通常 10s）会导致资源创建直接失败。 |
| **Webhook Denials** | reqson | $r_{deny}=\dfrac{\Delta N_{deny}}{\Delta t}$，按 $(type,\ op,\ reason)$ 分线 | 准入被拒的速率，理想恒为 0，阈值 **> 0.1/s 标橙**。有值说明有人提交了不合法的 DynamoGraphDeployment，拒绝原因直接指出被触发的校验规则 —— 排查"YAML apply 上去没生效"的第一站。 |

### Row 3 · Resource Inventory（资源清单，2 项）

| 参数名                         | 单位                          | 计算公式                                            | 详细解释                                                                  |
| --------------------------- | --------------------------- | ----------------------------------------------- | --------------------------------------------------------------------- |
| Resource Inventory by State | short（个，堆叠）                 | $N_{res}(t)$，按 $(type,\ ns,\ status)$ 分组求和，画成时序 | 各状态资源数量的变化过程。ready 那一层占满、其他层为 0 为好；出现并停留在 pending / failed 层 = 有部署卡住。 |
| Resource Count by State     | short（个，stat 取 lastNotNull） | $N_{res}$，按 $(type,\ status)$ 分组求和（瞬时快照）        | 当前各状态的资源数，是上一项的即时快照。                                                  |
...
| 参数名                                | 单位                   | 计算公式                                                    | 详细解释                  |
| ---------------------------------- | -------------------- | ------------------------------------------------------- | --------------------- |
| **Reconciliation Success Rate**    | percent（0~100，gauge） | $S_{rec}=\dfrac{r_{rec}(success)}{r_{rec}}\times 100\%$ | reconcile 成功率，理想 100。 |
| **Webhook Admission Success Rate** | percent（0~100，gauge） | $S_{wh}=\dfrac{r_{wh}(allowed)}{r_{wh}}\times 100\%$    | 准入放行率                 |
