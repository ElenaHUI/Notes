...
→ 细节见 [[秋招/面经/百度 AI infra面经|百度面经 Q1 / Q19]]、[[秋招/面经/商汤 大模型系统工程师面经|商汤面经 Q1-Q9]]、[[kimi2.6 PD分离部署记录]]
...
→ 细节见 [[LLM Prometheus 性能分析报告生成Skill]]、[[Dynamo/Dynamo监控链路搭建/针对Dynamo v1.3.0的Grafana看板]]
...
→ 细节见 [[DeepSeek Harness Web UI 离线包构建流程]]、[[秋招/面经/百度 AI infra面经|百度面经 Q15-Q18]]、[[AI提效/定时巡检agent]]
...
| 单模型生产配比 | Kimi-K2.6 **80 卡**（48P + 32D） | 说明单个模型的部署体量，不是平台总量 |
| 平台总资源 | **21 机 / 336 张 PPU 卡**，利用率 **100%**（0 空闲） | 说明整体体量：全平台 336 卡打满，而非只有一套 80 卡 |
| 最大占卡模型 | DeepSeek-V4-Flash **144 卡**（43%） | 说明多模型间的容量规划与资源治理 |
...
> [!info] PPU 资源看板快照（"你们平台多大"的标准答案）
> - 总量：**21 台机器 / 336 卡**，**13 个**大模型服务，利用率 **100%**（0 空闲）。
> - 占卡分布：DeepSeek-V4-Flash 144（43%）> Qwen3.8-27B 64（19%）≈ Kimi-K2.6 64（19%）> GLM-5.2 32（10%）> Qwen3.5-397B-A17B-INT8 16（5%）> Qwen3-Coder-Next 8（2%）> Qwen2.5-Coder-7B-Instruct 2（1%）。
> - 口径：**80 卡** = Kimi-K2.6 单模型生产配比（3P2D）；**336 卡** = 平台总量。被问"部署了多少卡"先报 336，再下钻到单模型 80 卡。

> [!warning] 待补数字（有就填，没有别编）
> - [ ] warmup 改造后 TTFT P99 实际下降多少 / 首请求耗时对比（见 [[Warmup效果对比]]）
> - [ ] 平台服务的内部用户数 / 日请求量
> - [ ] 上线周期从多久缩短到多久（GitOps 收益的量化）
> - [ ] 单次故障定位平均耗时、可用性数字
> - [ ] 核对 PPU 看板口径：看板 Kimi-K2.6 为 64 卡，与 3P2D=80 卡不一致（看板是单环境快照还是配比已调整？）；看板"13 个大模型"与正文"7 个在线模型"是否同口径（含测试/小模型？）
> - [ ] 模型名以哪个为准：Qwen3.6-27B 还是 Qwen3.8-27B（看板显示 Qwen3.8-27B）
...
| 红区为什么要离线包？ | 无外网 → 依赖闭包 + 运行时全部预打包，自解压单文件投放 | [[DeepSeek Harness Web UI 离线包构建流程]] |
